...
contract AttackNaughtCoin {
    ...
    function attack(address _naughtCoin) public {
        naughtCoin = NaughtCoin(_naughtCoin);
        uint myBalance = naughtCoin.allowance(owner, address(this));

        require(
            myBalance > 0,
            "Approve some tokens to this address then call this function"
        );
